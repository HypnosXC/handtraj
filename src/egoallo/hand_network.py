...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, config: TransformerBlockConfig) -> None:
        ...
        if config.include_xattn:
            ...
            self.xattn_shared = nn.Linear(config.d_latent, config.d_latent, bias=False)
    ...
    def forward(
        ...
        # --- 1. Self-attention (改为 Pre-Norm) ---
        # 理论依据：子空间投影。在进入计算前先 layernorm
        x = x + self._sattn(self.layernorm1(x), attn_mask)

        # Include conditioning.
        if config.include_xattn:
            assert cond is not None
            # 注意：这里对 x 使用 xattn_layernorm
        ...
        if self.fix_lambda:
            x = self._ista_update_fix_lambda_step_size(
                x_ff_in, config.ista_step_size, config.ista_lambd
            )
        else:
            x = self._ista_update(x_ff_in)
    ...
    def _ista_update_fix_lambda_step_size(self, x,ista_step_size,ista_lambd) -> torch.Tensor:
        D = self.dict_weight
