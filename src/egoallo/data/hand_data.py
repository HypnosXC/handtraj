...
class HandHdf5EachDataset(torch.utils.data.Dataset[HandTrainingData]):
    ...
    def __init__(
        ...
        if dataset_name=="dexycb":
            ...
            self.video_root = "/data-share/share-folder/handdata/preprocessed/interhand26m/picked_videos"
        elif dataset_name=="arctic":
            self._hdf5_path = "/data-share/share-folder/handdata/preprocessed/arctic/arctic.hdf5"
            self.video_root = "/data-share/share-folder/handdata/preprocessed/arctic/picked_videos"
        elif dataset_name == "ho3d":
            self.video_root = "/data-share/share-folder/handdata/preprocessed/ho3d/picked_videos"
            if split == "train":
                self._hdf5_path = "/data-share/share-folder/handdata/preprocessed/ho3d/ho3d_train.hdf5"
            elif split == "test":
                self._hdf5_path = "/data-share/share-folder/handdata/preprocessed/ho3d/ho3d_evaluation.hdf5"
    ...
    def get_video_name(self, index: int) -> str:
        group_name, _, _ = self._mapping[index]
        dataset = self.archive[group_name]
        video_name = dataset['video_name'][()].decode('utf-8')
        return video_name
...
class HandHdf5Dataset(torch.utils.data.Dataset[HandTrainingData]):
    ...
    def get_video_name(self, index: int) -> str:
        for ds in self.dataset.datasets:
            if index < len(ds):
                return ds.get_video_name(index)
            else:
                index -= len(ds)
...
if __name__ == "__main__":    
    ...
    dataset = HandHdf5Dataset(split='train', dataset_name="dexycb", vis=True, subseq_len=64, clip_stride=64, min_len=32, speed_augment=None)
    dataset.visualize_joints_in_rgb(7469, out_dir=f"vis_dexycb_7469", resize=(512,512))
    dataset.visualize_joints_in_rgb(7469, out_dir=f"vis_dexycb_from_mano_results_7469", resize=(512,512),from_mano=True)
