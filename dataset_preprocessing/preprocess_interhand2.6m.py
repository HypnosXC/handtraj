...
class Interhand2_6M(torch.utils.data.Dataset):
    def __init__(
        ...
        self._subseq_len = 64
        # self.cap_format = "Capture{}"
        # self.cam_format = "cam{}"
        # self.N = len(self.record_split_seqs_32['left']) + len(self.record_split_seqs_32['right'])

        # self.img_root = img_dir
        # self.split = split

        # with open(os.path.join(self.anno_root, split, 'InterHand2.6M_train_MANO_NeuralAnnot.json')) as f:
        #     self.mano_data = json.load(f)

        # with open(os.path.join(self.anno_root, split, 'InterHand2.6M_train_joint_3d.json')) as f:
    ...
    def __getitem__(self, index: int):
        ...
        kwargs['mano_pose'] = torch.FloatTensor(this_mano_data['pose'])
        # mean of betas
...
def save_splits2hdf5(hdf5_path, compression: str = 'gzip'):
    ...
    # mano_model_folder = "/home/yunqili/code/hamer/_DATA/data/mano"
    ...
    # mano_layer_15_coeff_left = ManoLayer(
    #     flat_hand_mean=False,
    #     ncomps=15,
    #     side='left',
    #     mano_root=mano_model_folder,
    # )
    # mano_layer_15_coeff_right = ManoLayer(
    #     flat_hand_mean=False,
    #     ncomps=15,
    #     side='right',
    #     mano_root=mano_model_folder,
    ...
    # for seq_dir in data_seq_list:
    #     full_seq_dir = os.path.join(hot3d_root_path, seq_dir)
    #     undistorted_img_seq_dir = os.path.join(undistorted_data_dir, seq_dir)
    #     os.makedirs(undistorted_img_seq_dir, exist_ok=True)
    #     data_provider = Hot3dDataProvider(
    #         sequence_folder=full_seq_dir,
    #         object_library=None,
    ...
    #     _device_data_provider = data_provider.device_data_provider
    #     for idx,timestamp_ns in enumerate(tqdm.tqdm(timestamps)):
    #         image_data = _device_data_provider.get_undistorted_image(timestamp_ns, stream_id)
    #         # save image_data(np.array) to undistorted_img_seq_dir
    #         img = image_data.image
    #         img_path = os.path.join(undistorted_img_seq_dir, f"{idx:06d}.png")
    #         iio.imwrite(img_path, image_data)
    # return

    str_dt = h5py.string_dtype('utf-8', None)
    # bad_data_file = "/public/datasets/handdata/interhand26m/anno/interhand_bad_data.txt"
    # cam_mano_dir = os.path.join("/public/datasets/handdata/interhand26m/anno/annotation", split, 'InterHand2.6M_train_camera_mano.json')
    # cam_joint_dir = os.path.join("/public/datasets/handdata/interhand26m/anno/annotation", split, 'InterHand2.6M_train_camera_joint.json')
    # capture_cnt = {
    #     "train":10,
    #     "val":1,
    #     "test":2
    ...
    shapes = {
        ...
        # 'rgb_frames': (64, 480, 640, 3),  # 64 frames, RGB images of size 480x640
    ...
    with h5py.File(hdf5_path, 'w') as f:
        for split in ['train', 'val', 'test']:
            ...
            # anno_root = os.path.join(interhand_anno_dir, split)
            # camera_file = os.path.join(anno_root, 'InterHand2.6M_train_camera.json')
            # mano_file = os.path.join(anno_root, 'InterHand2.6M_train_MANO_NeuralAnnot.json')
            # joint_file = os.path.join(anno_root, 'InterHand2.6M_train_joint_3d.json')
            ...
            #             frame_cnt = len(os.listdir(cam_dir))
            #             sub_seq_num = frame_cnt//64
            #             for sub_seq_idx in range(sub_seq_num):
            #                 if frame_cnt-sub_seq_idx*64<32:
            #                     continue
            #                 this_mapping.append((cap_str, seq, cam, sub_seq_idx))
            ...
            for idx, P_idx in enumerate(tqdm.tqdm(range(N), desc=f"Processing {split} split")):
                ...
                ds_mano_poses[idx] = sample['mano_pose'].unsqueeze(1)
                ds_mano_joint_3d = grp['mano_joint_3d']
                ds_mano_joint_3d[idx] = sample['mano_joint_3d'].unsqueeze(1)
                ...
                ds_mask = grp['mask']
                ds_mask[idx] = sample['mask'].numpy().astype(np.uint8)
