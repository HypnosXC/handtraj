...
def visualize_joints_in_rgb(traj:HandDenoiseTraj,
    ...
    # 创建视频写入对象
    out = cv2.VideoWriter(out_dir+"/infer_dexycb.mp4", fourcc, fps, (width, height))
...
class Args:
    checkpoint_dir: Path = Path("../../../xuchen/handtraj/experiments/hand_train_cond_wrist_motion_more_data/v2/checkpoints_650000/")#Path("./egoallo_checkpoint_april13/checkpoints_3000000/")
...
def main(args: Args) -> None:
    ...
    if test_hamer == True:
        ...
        for i in range(N):
            ...
            subseq_len = gt.mask.sum().cpu().numpy().astype(int)
            hamer_out = []
            ...
            # breakpoint()
            # errors[var]+= torch.norm((masked_train_joints - masked_pred_joints), dim=-1).mean(dim=-1).sum().cpu().numpy()
            joint_errors+= torch.norm((gt_values["joint_3d"] - hamer_out["keypoints_3d"]), dim=-1).mean().cpu().numpy()
            ...
            for train_batch in tqdm(dataloader):
                ...
                for var in errors.keys():
                    if var  != "3D_joints" and var  !="mano_poses":
                        ...
                        if var == "3D_joints":
                            # breakpoint()
                            # mask invalid joints with train_batch.mask (batch_size,64)
                            # train_batch.mano_joint_3d to masked_train_joints (batch_size,64,21,3)
                            masked_train_joints = (train_batch.mano_joint_3d * train_batch.mask[:,:,None,None]).to(device)
                            masked_pred_joints = joints_pred * train_batch.mask[:,:,None,None].to(device)
                            # sum all valid counts mask (64,64) to int
                            valid_counts = train_batch.mask.sum().cpu().numpy()/64
                            # errors[var]+= torch.sqrt(((train_batch.mano_joint_3d.to(device) - joints_pred)**2).sum(dim=-1)).mean(dim=-1).sum().cpu().numpy()
                            errors[var]+= torch.norm((masked_train_joints - masked_pred_joints), dim=-1).mean(dim=-1).sum().cpu().numpy()/valid_counts
